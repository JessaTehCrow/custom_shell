...
def run(*command:str,**other:str):
    "Runs a non shell command as background process"
    __help__ = "[B]Command[G] Any cmd command"
...
def output(ID:int):
    "Shows output for a finished process"
    __help__ = "[B]ID[G] is the ID of the process.\n[G](Can be found using 'process data')"
    handler.output(ID)
